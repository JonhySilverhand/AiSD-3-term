...
int main()
{
	...
	vector <int> sequence(n); //элементы последовательности
	vector<int> maxLen(n);    //максимальная длина подпоследовательности
	vector<int> next(n);      //индексы элементов подпоследовательности
